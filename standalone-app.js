// Pi Calculator Pro - Standalone Application
// Multi-core Pi calculation with modern UI - No external dependencies
...
class PiCalculatorApp {
    ...
    async init() {
        ...
        console.log('Pi Calculator Pro Standalone initialized');
    }
    ...
    async startCalculation() {
        ...
        const numWorkers = this.systemInfo.cores || 4;
    ...
    getInlineWorkerCode() {
        return `
// Pi Calculator Web Worker - Standalone with embedded Pi data
...
function getPiDigitsFromWorker(count) {
    if (count <= 1000) {
        return PI_1000.substring(0, count + 2);
    }
    
    // For larger counts, repeat the pattern
    let result = PI_1000;
    while (result.length - 2 < count) {
        result += PI_1000.substring(2); // Skip the "3." part
    }
...
self.onmessage = function(e) {
    ...
    validatePiInput(input) {
        // Remove non-digit characters except for the first '3.'
        const value = input.value;
        if (value.startsWith('3.')) {
            const decimalPart = value.substring(2).replace(/[^0-9]/g, '');
            input.value = '3.' + decimalPart;
        } else {
    ...
    exportResult() {
        ...
        link.href = url;
        link.download = `pi-calculation-${metadata.digits}-digits.json`;
        link.click();
    ...
    updateHistoryDisplay() {
        ...
        historyList.innerHTML = this.calculationHistory.slice(0, 10).map((calc, index) => `
            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                <div class="flex justify-between items-center">
                    <div>
                        <div class="font-medium text-gray-900 dark:text-white">${calc.digits.toLocaleString()} decimaler</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">${calc.algorithm} • ${new Date(calc.timestamp).toLocaleString()}</div>
                    </div>
                    <div class="flex space-x-2">
                        <button onclick="app.copyHistoryResult(${index})" class="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path>
                            </svg>
                        </button>
                        <button onclick="app.deleteHistoryItem(${index})" class="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    copyHistoryResult(index) {
        const calculation = this.calculationHistory[index];
        navigator.clipboard.writeText(calculation.pi).then(() => {
            this.showToast('Resultat kopierat från historik!', 'success');
        }).catch(() => {
            this.showToast('Kunde inte kopiera', 'error');
        });
    }
    ...
    deleteHistoryItem(index) {
        ...
        this.updateHistoryDisplay();
        this.saveHistory();
        this.showToast('Borttaget från historik', 'info');
    }
    ...
    async saveHistory() {
        try {
            localStorage.setItem('piCalculationHistory', JSON.stringify(this.calculationHistory));
        } catch (error) {
    ...
    async loadHistory() {
        try {
            const saved = localStorage.getItem('piCalculationHistory');
            if (saved) {
    ...
    async saveSettings() {
        try {
            ...
            this.showToast('Kunde inte spara inställningar', 'error');
        }
    }
    ...
    applySettings() {
        ...
        // Update max decimals input
        const digitsInput = document.getElementById('digitsInput');
        if (digitsInput) {
            digitsInput.max = this.settings.maxDecimals;
        }
    }
...
function comparePi() {
    ...
    // Remove '3.' from both strings for comparison
    const userDecimals = userInput.replace(/^3\.?/, '');
    const calcDecimals = calculatedPi.replace(/^3\.?/, '');
    
    // Find matching digits
    let matchingDecimals = 0;
    for (let i = 0; i < Math.min(userDecimals.length, calcDecimals.length); i++) {
        if (userDecimals[i] === calcDecimals[i]) {
            matchingDecimals++;
        } else {
            break;
        }
    ...
    document.getElementById('correctDecimals').textContent = matchingDecimals;
    document.getElementById('userDecimals').textContent = userDecimals.length;
    document.getElementById('accuracy').textContent = userDecimals.length > 0 ? 
        ((matchingDecimals / userDecimals.length) * 100).toFixed(2) + '%' : '0%';
    
    // Show detailed result indicator
    const resultDiv = document.getElementById('comparisonResult');
    const resultText = document.getElementById('comparisonResultText');
    
    resultDiv.classList.remove('hidden');
    
    if (matchingDecimals === userDecimals.length && userDecimals.length > 0) {
        resultDiv.className = 'mt-4 p-3 rounded-lg text-center font-medium bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200';
        resultText.innerHTML = `🎉 <strong>PERFEKT MATCH!</strong><br>Alla ${userDecimals.length} decimaler stämmer!`;
        app.showToast('Perfekt match!', 'success');
    } else if (matchingDecimals === 0) {
        resultDiv.className = 'mt-4 p-3 rounded-lg text-center font-medium bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200';
        resultText.innerHTML = `❌ <strong>INGA MATCHANDE DECIMALER</strong><br>Kontrollera din inmatning`;
        app.showToast('Inga matchande decimaler', 'error');
    } else {
        const percentage = ((matchingDecimals / userDecimals.length) * 100).toFixed(2);
        resultDiv.className = 'mt-4 p-3 rounded-lg text-center font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200';
        resultText.innerHTML = `📊 <strong>${matchingDecimals} av ${userDecimals.length} decimaler stämmer</strong><br>Noggrannhet: ${percentage}%`;
